...
import { FALLBACK_TOKENS, toNano, fromNano, ROUTER_V2, PTON_V2 } from '@/lib/stonfi';
...
export function SwapPage() {
  ...
  // Balance: prefer live wallet data, fall back to token default
  const walletSendToken = walletTokens.find((t) => t.address === sendToken.address);
  const sendBalance = walletSendToken?.balance ?? sendToken.balance ?? '0';

  const walletRecvToken = walletTokens.find((t) => t.address === recvToken.address);
  const recvBalance = walletRecvToken?.balance ?? recvToken.balance ?? '0';
  ...
  const handleSwap = async () => {
    ...
    try {
      /*
       * ── Production swap with @ston-fi/sdk v2 ─────────────────────────────
       *
       ...
       *
       * ─────────────────────────────────────────────────────────────────────
       * Current placeholder: sends to router WITHOUT payload.
       * The router will reject it (no funds lost), but the wallet will
       * prompt so you can confirm the TonConnect flow works end-to-end.
       * Replace with the block above once you wire in the SDK client.
       */
      ...
      await tonConnectUI.sendTransaction({
        validUntil: Math.floor(Date.now() / 1000) + 600,
        messages: [{
          address: ROUTER_V2,           // ← correct router address
          amount:  amountNano,
          // payload: txParams.body?.toBoc().toString('base64'),
        }],
      });

      toast.success(`Swap submitted: ${sendAmount} ${sendToken.symbol} → ${recvAmount} ${recvToken.symbol}`);
      setSendAmount('');
    } catch (err: any) {
  ...
  const btnLabel = !connected ? 'Connect wallet to swap'
    : !sendAmount ? 'Enter an amount'
    : swapping    ? 'Confirming...'
    : `Swap ${sendToken.symbol} → ${recvToken.symbol}`;

  return (
    <div style={{ maxWidth: 480, margin: '0 auto' }}>
      {showSendModal && (
        <TokenModal
          tokens={tokens} walletTokens={walletTokens}
        ...
        <div style={{ padding: 20 }}>
          ...
          <div style={{ background: '#f7f9fc', borderRadius: 16, padding: '14px 16px 12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              {connected && (
                <button
                  onClick={() => setSendAmount(parseFloat(sendBalance).toString())}
                ...
                >
                  ...
                  {parseFloat(sendBalance).toFixed(6)}
                </button>
              )}
            </div>
          ...
          <div style={{ background: '#f7f9fc', borderRadius: 16, padding: '14px 16px 12px', marginTop: -6 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 13, color: '#aaa' }}>
                {connected ? `${parseFloat(recvBalance).toFixed(4)} bal` : '—'}
              </span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {sendAmount && recvAmount && (
            <div style={{ marginTop: 12, padding: '12px 14px', background: '#f7f9fc', borderRadius: 12 }}>
              {[
                { label: 'Rate', value: `1 ${sendToken.symbol} ≈ ${(parseFloat(recvAmount) / parseFloat(sendAmount)).toFixed(6)} ${recvToken.symbol}`, color: '#111' },
                { label: 'Price impact', value: `${parseFloat(priceImpact).toFixed(2)}%`, color: parseFloat(priceImpact) > 1 ? '#F59E0B' : '#22C55E' },
                { label: `Min received (${slippage}% slippage)`, value: `${minReceived} ${recvToken.symbol}`, color: '#111' },
                { label: 'Network fee', value: '~0.05–0.1 TON', color: '#111' },
          ...
          >
            {swapping
              ? <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><Spinner size={18} color="white" /> Confirming on TON...</span>
              : btnLabel}
          </button>
        </div>
      </div>
