...
export function StakePage() {
  ...
  // STON staking contract address
  const STAKING_CONTRACT = 'EQDmkj65Ab_m0aZaW8IpKw4kYqIgITw_HRstYEkVQ6NIYCyT';
  ...
  const handleStake = async () => {
    ...
    try {
      const amountNano = String(Math.round(parseFloat(stakeAmt) * 1e9));

      await tonConnectUI.sendTransaction({
        validUntil: Math.floor(Date.now() / 1000) + 600,
        messages: [{ address: STAKING_CONTRACT, amount: amountNano }],
      });
