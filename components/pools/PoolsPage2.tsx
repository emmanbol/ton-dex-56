...
function AddLiquidityModal({ pool, onClose }: { pool: Pool; onClose: () => void }) {
  ...
  const ratio = pool.token0.price && pool.token1.price ? pool.token0.price / pool.token1.price : 1;
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      // In production: use @ston-fi/sdk to build provide_lp transaction
      await tonConnectUI.sendTransaction({
        validUntil: Math.floor(Date.now() / 1000) + 600,
        messages: [{ address: pool.address, amount: String(Math.round(parseFloat(amt0) * 1e9)) }],
      });
      setDone(true);
      toast.success('Liquidity added successfully!');
    } catch (err: any) {
      if (!err?.message?.includes('declined')) toast.error('Transaction failed');
    } finally {
      setAdding(false);
    }
  };
      ...
      <div style={{ background: 'white', borderRadius: 22, width: '100%', maxWidth: 450, padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
        ...
        {done ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 6 }}>Liquidity Added!</div>
          ...
          <>
            {[{ token: pool.token0, value: amt0, onChange: setAmt0 }, { token: pool.token1, value: amt1, readonly: true }].map(({ token, value, onChange, readonly }, i) => (
              <div key={i} style={{ background: '#f7f9fc', borderRadius: 14, padding: '12px 14px', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <DoubleTokenIcon t0={token} t1={token} size={0} />
            ...
            <div style={{ background: '#f0f9ff', borderRadius: 12, padding: 12, marginBottom: 14, fontSize: 12 }}>
              {[
                ...
                { label: 'APR (current)', value: `${pool.apr.toFixed(1)}%`, color: '#22C55E' },
            ...
            >
              {adding ? 'Adding liquidity...' : !wallet ? 'Connect wallet' : !amt0 ? 'Enter an amount' : 'Add Liquidity'}
            </button>
          </>
        )}
      </div>
...
export function PoolsPage() {
  ...
  const filtered = pools
    .filter((p) => {
      const matchSearch = p.token0.symbol.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        ...
        <div style={{ padding: '18px 20px', borderBottom: '1px solid #f0f2f5' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
            ...
            <button
              onClick={() => connected ? setSelectedPool(pools[0]) : undefined}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', background: connected ? 'linear-gradient(135deg,#0098EA,#0077BB)' : '#f0f2f5', color: connected ? 'white' : '#aaa', border: 'none', borderRadius: 12, fontWeight: 700, fontSize: 14, cursor: connected ? 'pointer' : 'default', fontFamily: 'inherit', boxShadow: connected ? '0 3px 12px rgba(0,152,234,.3)' : 'none' }}
            >
              + Add Liquidity
            </button>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 120px', gap: 8, padding: '10px 20px', background: '#fafbfc', borderBottom: '1px solid #f0f2f5' }}>
          {[['Pool', null], ['TVL', 'tvl'], ['24h Volume', 'volume24h'], ['APR', 'apr'], ['Action', null]].map(([label, key]) => (
